...
function renderSeeds(){const s=liveSeeds(),el=document.getElementById('seedList');el.innerHTML=s.map((r,i)=>`<div class="seed-row"><div class="rank">${i+1}</div><div><div class="seed-team">${r.team}</div><div class="seed-role">${i<3?'group winner':'runner-up'} · ${r.pts} pts · ${r.diff>=0?'+':''}${r.diff}</div></div><div class="${i===0?'bye':''}">${i===0?'SF BYE':''}</div></div>`).join('')}
